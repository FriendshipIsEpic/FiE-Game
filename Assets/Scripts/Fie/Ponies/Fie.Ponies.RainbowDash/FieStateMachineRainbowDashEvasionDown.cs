...
{
	public class FieStateMachineRainbowDashEvasionDown : FieStateMachineGameCharacterBase
	{
		...
		private const float EVASION_DURATION = 0.15f;
		...
		public override void updateState<T>(ref T gameCharacter)
		{
			if (gameCharacter is FieRainbowDash)
			{
				...
				case EvasionState.PREPARE:
				{
					...
					TrackEntry trackEntry = rainbowDash.animationManager.SetAnimation(25, isLoop: false, isForceSet: true);
					if (trackEntry != null)
					{
		...
		private void setEvasion(FieRainbowDash rainbowDash, Spine.Event e)
		{
			...
			if (rainbowDash.externalInputForce <= 0.25f)
			{
				vector = Vector3.down;
			}
			vector.Normalize();
			...
			_evasionTweener.InitTweener(0.15f, rainbowDash.position, _evasionTargetPos);
